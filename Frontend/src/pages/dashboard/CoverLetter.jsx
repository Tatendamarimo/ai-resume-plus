...
function CoverLetter({ resumeInfo }) {
  ...
  const handleResumeSelect = async (e) => {
    ...
    if (resumeId) {
      try {
        const result = await getResumeData(resumeId);
        ...
        setFormData(prev => ({
          ...prev,
          candidateName: data.firstName && data.lastName ? `${data.firstName} ${data.lastName}` : prev.candidateName,
          background: data.jobTitle || prev.background,
          skills: data.skills?.map(s => s.name).join(', ') || prev.skills,
          // You might want to extract experience from work history if available
          experience: data.experience?.map(exp => `${exp.title} at ${exp.companyName}`).join(', ') || prev.experience
        }));
  ...
  // Auto-populate from prop if provided (legacy support)
  useEffect(() => {
    if (resumeInfo) {
      setFormData(prev => ({
        ...prev,
        ...
        <div className="grid lg:grid-cols-2 gap-6 md:gap-8">
          ...
          >
            ...
            <div className="space-y-5">
              ...
              <div className="bg-gray-700/30 p-4 rounded-xl border border-gray-600/50 mb-6">
                <label className="block text-sm font-medium text-primary mb-2">
                  Auto-fill from Resume (Optional)
                </label>
                <select
